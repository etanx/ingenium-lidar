...
% If there are no more frames available to read, throw an exception
if ~hasFrameLite(obj)
    msg = message('multimedia:videofile:EndOfFile');
...
% If an error was generated when reading ahead, then throw that exception.
throwNonEofException(obj);

% At this point, StoredFrame cannot be empty
varargout{1} = VideoReader.convertToOutputFormat( obj.StoredFrame.Data, ...
                                                  get(obj, 'VideoFormat'), ...
                                                  outputformat, ...
...
try
    obj.StoredFrame = [];
    obj.StoredFrame = readNextFrame(getImpl(obj));
    obj.InternalCurrentTime = [];
catch ME
