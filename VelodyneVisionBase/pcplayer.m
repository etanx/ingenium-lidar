...
classdef pcplayer < vision.internal.EnforceScalarHandle
    ...
    methods
        ...
        function view(this, varargin)
            ...
            % colors or intensities stored in the pointCloud object,
            % ptCloud.
            %
            ...
            %
            % VIEW(player,xyzPoints,colorValue) displays points at the
            % locations that are contained in the xyzPoints matrix with
            % colors specified by colorValue. To specify the same color for
            % all points, colorValue must be a color string or a 1-by-3 RGB
            % vector. To specify a different color for each point,
            % colorValue must be an M-by-3 or M-by-N-by-3 matrix containing
            % RGB values for each point.
            %
            % VIEW(player,xyzPoints,colorMapValue) displays points at the
            % locations that are contained in the xyzPoints matrix with
            % colors from the current colormap. colorMapValue must be a
            % vector or an M-by-N matrix containing values that are
            ...
            [X,Y,Z,C,map] = pcplayer.parseInputs(varargin{:});
    ...
    methods(Hidden, Static, Access = protected)
        ...
        function [X, Y, Z, C, map] = parseInputs(varargin)              
            ...
            if isa(varargin{1}, 'pointCloud')
                ...
                narginchk(1,2);
                [X, Y, Z, C, map] = vision.internal.pc.validateAndParseInputsXYZC(mfilename, varargin{:});
            end
